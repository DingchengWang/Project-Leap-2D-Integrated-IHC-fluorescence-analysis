...
def _run_workspace(project_root: Path, forwarded: list[str]) -> int:
    original_image = project_root / "Sample Image"
    result = project_root / "Result"
    runtime_root = project_root / "Analysis Package" / "Run State"
    ...
    with project_workspace_lock(runtime_root):
        ...
        try:
            ...
            print(
                ...
                "The validated Result files were kept. The Sample Image files "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if _contains_path_override(forwarded):
        raise ValueError(
            "The packaged launcher always uses its own Sample Image and Result "
