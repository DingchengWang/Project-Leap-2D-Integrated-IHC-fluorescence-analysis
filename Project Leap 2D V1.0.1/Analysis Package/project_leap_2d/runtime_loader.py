...
from .runtime_manifest import (
    ...
    PROCESSES_TRIM_RESOURCE_SHA256,
    MODULE_ORDER,
...
def load_runtime() -> types.ModuleType:
    ...
    with _LOAD_LOCK:
        ...
        try:
            ...
            trim_path = groovy_path.with_name("processes_trim.groovy")
            trim_bytes = trim_path.read_bytes()
            observed_trim_sha = _sha256(trim_bytes)
            if observed_trim_sha != PROCESSES_TRIM_RESOURCE_SHA256:
                raise RuntimeError(
                    "Fiji Processes trim resource integrity check failed: "
                    f"{observed_trim_sha}"
                )
            runtime.FIJI_GROOVY_SCRIPT = (
                groovy_bytes.decode("utf-8") + "\n" + trim_bytes.decode("utf-8")
            )
