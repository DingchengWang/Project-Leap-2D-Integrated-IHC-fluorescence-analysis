...
start_download() {
  /usr/bin/curl \
    --fail --location --silent --show-error --proto '=https' --tlsv1.2 \
    --output "$2" "$1" &
  STARTED_DOWNLOAD_PID=$!
}
...
download_verified() {
  start_download "$1" "$3"
  UV_DOWNLOAD_PID=$STARTED_DOWNLOAD_PID
  download_status=0
  wait "$UV_DOWNLOAD_PID" || download_status=$?
  UV_DOWNLOAD_PID=
  test "$download_status" -eq 0 || fail "$4 download failed."
  verify_download "$@"
}

stop_downloads() {
  # curl is a direct child, so every writer is stopped and reaped before rollback.
  # The shell job table also covers a signal between spawning curl and saving $!.
  # Downloads are the installer's only background jobs.
  active_download_pids=$(jobs -p)
  for download_pid in $active_download_pids; do
    /bin/kill -TERM "$download_pid" 2>/dev/null || true
  done
  for download_pid in $active_download_pids; do
...
finish_parallel_downloads() {
  download_status=0
  wait "$MODEL_DOWNLOAD_PID" || download_status=$?
  MODEL_DOWNLOAD_PID=
  test "$download_status" -eq 0 || fail "Cellpose cpsam_v2 model download failed."
  verify_download \
    "$PROJECT_LEAP_CELLPOSE_MODEL_URL" "$PROJECT_LEAP_CELLPOSE_MODEL_SHA256" \
    "$RELEASE_DIR/Models/cpsam_v2" "Cellpose cpsam_v2 model"
  download_status=0
  wait "$FIJI_DOWNLOAD_PID" || download_status=$?
  FIJI_DOWNLOAD_PID=
  test "$download_status" -eq 0 || fail "Fiji download failed."
  verify_download \
    "$PROJECT_LEAP_FIJI_URL" "$PROJECT_LEAP_FIJI_SHA256" "$FIJI_ARCHIVE" "Fiji"
}

PROJECT_VERSION=$(/bin/cat "$PROJECT_DIR/Analysis Package/VERSION")
...
DOCTOR_CACHE_DIR="$PROJECT_DIR/Analysis Package/Run State/matplotlib"
INSTALL_COMPLETE=0
REPAIR_PREPARING=0
REPAIR_ACTIVE=0
REPAIR_ATTEMPTED=0
...
acquire_environment_lock() {
  ...
  if test "${PROJECT_LEAP_REPAIR_LOCK_HELD+x}" = x; then
    test "$PROJECT_LEAP_REPAIR_LOCK_HELD" = "$USAGE_LOCK_FILE" ||
      fail "the inherited repair lock names a different support directory."
    # With no pathname, macOS stat uses fstat(stdin). /dev/fd/9 instead reports
    # a devfs device number, which cannot identify the underlying lock file.
    inherited_lock_identity=$(/usr/bin/stat -f '%d:%i' <&9 2>/dev/null) ||
      fail "the inherited repair lock descriptor is unavailable."
    expected_lock_identity=$(/usr/bin/stat -L -f '%d:%i' "$USAGE_LOCK_FILE" 2>/dev/null) ||
      fail "the inherited repair lock file is unavailable."
    test "$inherited_lock_identity" = "$expected_lock_identity" ||
      fail "the inherited repair lock descriptor names a different file."
    /usr/bin/lockf -s -t 0 9 ||
      fail "the inherited repair lock could not be acquired."
    LOCK_ACQUIRED=1
    return
  fi
...
analysis_is_running() {
  analysis_lock="$PROJECT_DIR/Analysis Package/Run State/locks/workspace.lock"
...
# These two independent downloads overlap the uv/Python installation chain.
# Only a confirmed build reaches this point; a healthy environment never downloads.
FIJI_ARCHIVE="$RELEASE_DIR/Downloads/Fiji.$PROJECT_LEAP_FIJI_ARCHIVE_KIND"
start_download "$PROJECT_LEAP_CELLPOSE_MODEL_URL" "$RELEASE_DIR/Models/cpsam_v2"
MODEL_DOWNLOAD_PID=$STARTED_DOWNLOAD_PID
start_download "$PROJECT_LEAP_FIJI_URL" "$FIJI_ARCHIVE"
FIJI_DOWNLOAD_PID=$STARTED_DOWNLOAD_PID
