...
def require_project_resources(project_dir: Path) -> None:
    model_dir = project_dir / "Analysis Package" / "project_leap_2d" / "resources" / "models"
...
def require_environment_content(
    args: argparse.Namespace,
    wheel_manifest: dict[str, Any],
    fiji_manifest: dict[str, Any],
    managed_python_manifest: dict[str, Any],
) -> None:
    require_virtual_environment_files(
        args.release_dir, managed_python_manifest["runtime"]
    )

    def check_python() -> None:
        require_managed_python_tree_integrity(
            args.release_dir, managed_python_manifest
        )

    def check_wheels() -> None:
        wheel_paths = require_wheel_file_integrity(
            args.lock_file, args.release_dir, wheel_manifest
        )
        require_environment_inventory(args.release_dir, wheel_paths)

    def check_cellpose() -> None:
        model = args.release_dir / "Models" / "cpsam_v2"
        if sha256_file(model) != args.cellpose_sha256:
            raise RuntimeError(
                "Cellpose model hash does not match the installation contract."
            )
    ...
    def check_fiji() -> None:
        ...
        if not any(path.is_dir() for path in java_roots):
            raise RuntimeError("Fiji does not contain its managed Java runtime.")
        require_fiji_tree_integrity(args.release_dir, fiji_manifest)

    # The installer holds the environment lock and does not mutate these trees
    # until every reader has finished. No third-party imports run in this phase.
    errors: list[Exception] = []
    with ThreadPoolExecutor(max_workers=min(4, os.cpu_count() or 1)) as executor:
        futures = [
            executor.submit(check)
            for check in (check_python, check_wheels, check_cellpose, check_fiji)
        ]
        for future in futures:
            try:
                ...
                errors.append(exc)
    # A filesystem/system failure must not be hidden by a concurrent integrity
    # mismatch, which would otherwise incorrectly authorize rebuilding.
    for error in errors:
        if isinstance(error, (MemoryError, PermissionError)) or (
            isinstance(error, OSError) and error.errno in NON_REPAIRABLE_ERRNOS
        ):
            raise error
    if errors:
        raise errors[0]
...
def main() -> int:
    ...
    try:
        if args.mode == "content":
            require_environment_content(
                args, wheel_manifest, fiji_manifest, managed_python_manifest
            )
        else:
        ...
        if exc.errno in NON_REPAIRABLE_ERRNOS:
            print(
