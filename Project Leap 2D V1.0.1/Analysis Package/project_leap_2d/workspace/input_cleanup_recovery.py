...
def _require_safe_recovery_root(runtime_root: Path, *, create: bool) -> Path:
    runtime_root = _require_real_directory(runtime_root, "Analysis Package/Run State")
    recovery_root = runtime_root / "recovery"
    if _lexists(recovery_root):
        if recovery_root.is_symlink() or not recovery_root.is_dir():
            raise RuntimeError(
                "Analysis Package/Run State/recovery must be a real directory, not a symlink"
            )
    elif create:
...
def _validated_payload(
    ...
    *,
    ...
    if Path(str(payload.get("original_image", ""))).resolve() != original_image:
        raise RuntimeError(
            "Input cleanup recovery journal belongs to another Sample Image"
        )
...
def begin_input_trash_transaction(
    *,
    ...
    original_image = _require_real_directory(original_image, "Sample Image")
    runtime_root = _require_real_directory(runtime_root, "Analysis Package/Run State")
...
def recover_input_trash_transaction(
    *,
    ...
    for entry in entries:
        ...
        if _lexists(staged):
            ...
            if _lexists(destination):
                raise RuntimeError(
                    f"Input cleanup recovery conflict in Sample Image: {name}"
                )
    ...
    for staged, destination, entry in moves:
        ...
        if _lexists(destination):
            raise RuntimeError(
                "Input cleanup recovery conflict in Sample Image: "
                f"{destination.name}"
            )
    ...
    *,
    ...
    for snapshot in snapshots:
        ...
        if raw_source.is_symlink() or raw_source.resolve().parent != original_image:
            raise RuntimeError(
                "Refusing to move an input outside Sample Image: "
