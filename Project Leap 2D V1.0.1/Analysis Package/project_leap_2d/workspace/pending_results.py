...
def _recover_interrupted_archive(result: Path, runtime_root: Path) -> None:
    ...
    expected_result = result.resolve()
    if (
        Path(str(payload.get("result", ""))).resolve() != expected_result
        or staging.resolve().parent != (runtime_root / "staging").resolve()
        or re.fullmatch(r"pending-[0-9a-f]{32}", staging.name) is None
        or target.resolve().parent != expected_result
        or PENDING_PATTERN.fullmatch(target.name) is None
    ):
        raise RuntimeError(
            "Pending recovery journal does not match the current Result and "
            "Analysis Package/Run State paths; the journal was kept unchanged."
        )
