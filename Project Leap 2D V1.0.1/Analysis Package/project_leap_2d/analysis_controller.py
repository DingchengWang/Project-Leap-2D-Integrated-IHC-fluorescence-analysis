...
def _main_locked(argv: list[str] | None = None) -> int:
    ...
    if "eGFP" not in paths:
        raise ValueError(
            ...
            "Run Analysis.command for mature GFAP-only analysis."
        )
    ...
    if any(
        ...
        raise ValueError(
            "Positive XYZ pixel calibration is required for fixed physical-depth Z selection "
            "and physical Soma and morphology thresholds"
        )
    z_step_um = float(metadata["DAPI"]["pixel_depth_um"])
    fixed_z_windows = plan_fixed_z_windows(reference_shape[0], z_step_um)
    if len(fixed_z_windows) != EXPECTED_Z_INTERVAL_COUNT:
        print("z_step_limit", flush=True)
        return 2
    ...
    print_terminal_stage(
        "02 | STACK LOADING AND FIXED Z PREPARATION",
        "Common method: load each split ZYX stack once, verify shared calibration, "
        "exclude 5.0 um at each physical end, and build five equally spaced "
        "9.5-um Z positions.",
    )
    print(
        f"Z-step={z_step_um:.6f} um; "
        f"end exclusion={fixed_z_windows[0].actual_guard_um:.6f} um "
        f"({fixed_z_windows[0].guard_slices} slices); "
        f"window={fixed_z_windows[0].actual_width_um:.6f} um "
        f"({fixed_z_windows[0].window_slices} slices)",
        flush=True,
    )
    for window in fixed_z_windows:
        print(
            f"  position {window.position_1based}/5: "
            f"Z={window.z0 + 1}-{window.z1 + 1}",
            flush=True,
        )
    ...
    chosen_specs = complete_candidate_specs(fixed_z_windows, structural_stacks)
    ...
    def print_candidate_row(
        ...
        print_terminal_event(
            ...
            f"global={completed_count:02d}/{len(chosen_specs)} | "
            f"window={window_position}/5 | "
            f"window_completed={completed_in_window:02d}/18 | "
    ...
    print_terminal_stage(
        ...
        "Common method: each position completes its DAPI/structure MAX projections "
        "and structural composite, followed by one serial Apple Metal/MPS "
        "Cellpose-SAM inference; masks are cached for that position's 18 candidates.\n"
    ...
    def print_shared_preparation(event: dict[str, object]) -> None:
        step_names = {
            "dapi_max": "DAPI MAX projection",
            "structural_max": "structural MAX projection",
            "structural_composite": "structural composite",
        ...
        print_terminal_event(
            f"Window {int(event['position_1based'])}/5 | "
            f"Z={int(event['z_start_1based'])}-{int(event['z_end_1based_inclusive'])} | "
            f"{step_names.get(step, step)} | status={event['status']}"
            f"{channel_text}"
        )
    ...
    candidate_contexts = build_candidate_window_contexts(
        ...
        projection_cache=projection_cache,
        structural_map_cache=structural_map_cache,
        progress_callback=print_shared_preparation,
    )
    context_groups: list[tuple[CandidateWindowContext, list[CandidateSpec]]] = []
    ...
    with ThreadPoolExecutor(
        ...
        precompute_futures: dict[object, dict[str, object]] = {}
        for _priority, metadata_row, function, job_args in precompute_jobs:
            metadata_row = dict(metadata_row)
            print_terminal_event(
                f"Window {int(metadata_row['position_1based'])}/5 | "
                f"Z={int(metadata_row['z_start_1based'])}-"
                f"{int(metadata_row['z_end_1based_inclusive'])} | "
                f"{metadata_row['step']} | status=started"
            )
            precompute_futures[
        ...
        for future in as_completed(precompute_futures):
            future.result()
            metadata_row = precompute_futures[future]
            print_terminal_event(
                f"Window {int(metadata_row['position_1based'])}/5 | "
                f"Z={int(metadata_row['z_start_1based'])}-"
                f"{int(metadata_row['z_end_1based_inclusive'])} | "
                f"{metadata_row['step']} | status=completed"
            )

        primary_jobs: list[tuple[int, CandidateSpec, CandidateWindowContext]] = []
        reuse_jobs: list[tuple[int, CandidateSpec, CandidateWindowContext]] = []
        ...
        def job_priority(job: tuple[int, CandidateSpec, CandidateWindowContext]) -> tuple:
        ...
        completed_by_window = {
            window.position_1based: 0 for window in fixed_z_windows
        }
        position_by_z_mode = {
            window.z_mode: window.position_1based for window in fixed_z_windows
        }
        for future in as_completed(future_positions):
            ...
            try:
                ...
                window_position = position_by_z_mode[
                    chosen_specs[position - 1].z_mode
                ]
                completed_by_window[window_position] += 1
                print_candidate_row(
                    result[1],
                    chosen_specs[position - 1],
                    completed_candidates,
                    completed_by_window[window_position],
                    window_position,
                    float(result[3]),
                )
    ...
    window_by_z_mode = {window.z_mode: window for window in fixed_z_windows}
    for spec, row in zip(chosen_specs, rows):
        window = window_by_z_mode[spec.z_mode]
        row.update(
            {
                "z_position_1based": window.position_1based,
                "z_position_local_best": False,
                "z_target_window_um": Z_WINDOW_TARGET_UM,
                "z_surface_guard_um": Z_SURFACE_GUARD_UM,
            }
        )
    ...
    print_terminal_stage(
        "05 | POSITION-LOCAL RANKING AND FIVE-POSITION SELECTION",
        "Common method: rank each position's 18 candidates using only same-position "
        "agreement, then compare the five position-best candidates using fixed DAPI, Whole, "
        "centrality, and optical-quality scores.",
    )
    ranking_started = time.perf_counter()
    best_global_positions: list[int] = []
    local_selection_details: list[dict[str, object]] = []
    for window in fixed_z_windows:
        global_positions = [
            index
            for index, spec in enumerate(chosen_specs)
            if spec.z_mode == window.z_mode
        ]
        if len(global_positions) != EXPECTED_PROFILES_PER_Z:
            raise AssertionError(
                f"Z position {window.position_1based} must contain exactly "
                f"{EXPECTED_PROFILES_PER_Z} candidates"
            )
        local_masks = [candidate_masks[index] for index in global_positions]
        local_rows = [rows[index] for index in global_positions]
        local_best_position, local_details = rank_position_production_candidates(
            local_masks,
            local_rows,
        )
        global_best_position = global_positions[local_best_position]
        best_global_positions.append(global_best_position)
        local_selection_details.append(local_details)
        rows[global_best_position].update(
            {
                "z_position_local_best": True,
                "z_position_selected": False,
                "z_actual_width_um": window.actual_width_um,
                "z_actual_guard_um": window.actual_guard_um,
            }
        )
        print_terminal_event(
            f"Window {window.position_1based}/5 local best | "
            f"candidate={int(rows[global_best_position]['candidate']):02d} | "
            f"Z={window.z0 + 1}-{window.z1 + 1} | "
            f"score={float(rows[global_best_position]['auto_selection_score']):.6f}"
        )

    if len(best_global_positions) != EXPECTED_Z_INTERVAL_COUNT:
        raise AssertionError("Exactly five position-local best candidates are required")
    position_best_candidates = [
        (candidate_masks[index], rows[index], chosen_specs[index])
        for index in best_global_positions
    ]
    position_projections = [
        projection_cache[projection_keys[index]]
        for index in best_global_positions
    ]
    position_selection = score_z_position_best_candidates(
        position_best_candidates=position_best_candidates,
        dapi_stack=dapi_stack,
        structural_stacks=structural_stacks,
        ...
        pixel_depth_um=z_step_um,
        compartment_config=CompartmentConfig(),
        validation_config=DapiNucleus3DConfig(),
        max_workers=_EFFECTIVE_DAPI_INVENTORY_CPU_WORKERS,
        calibration_source=str(
            metadata["DAPI"].get("pixel_depth_source") or "unknown"
        ),
    )
    for position_quality, global_position in zip(
        position_selection["positions"],
        best_global_positions,
    ):
        quality = dict(position_quality)
        position_best_row = rows[global_position]
        position_best_row["z_position_quality"] = quality
        for field_name, field_value in quality.items():
            position_best_row[f"z_position_{field_name}"] = field_value
        position_best_row["z_position_nuclear_score"] = quality["nucleus_score"]
        position_best_row["z_position_center_score"] = quality["centrality_score"]
        position_best_row["z_position_selection_inventory"] = position_selection[
            "selection_inventory"
        ]
        print_terminal_event(
            f"Window {int(quality['position_index']) + 1}/5 quality | "
            f"valid={bool(quality['valid'])} | "
            f"analyzable_cells={int(quality['unique_nucleus_count'])} | "
            f"linked_nuclei={int(quality['associated_nucleus_count'])} | "
            f"whole_components={int(quality['whole_component_count'])} | "
            f"complete_nuclei={int(quality['complete_nucleus_count'])} | "
            f"axial_retention="
            f"{float(quality['mean_nucleus_axial_retention_fraction']):.6f} | "
            f"nucleus_association={float(quality['association_score']):.6f} | "
            f"center_rms_um={float(quality['nucleus_center_rms_um']):.6f} | "
            f"yield={float(quality['yield_score']):.6f} | "
            f"nucleus={float(quality['nucleus_score']):.6f} | "
            f"whole={float(quality['whole_score']):.6f} | "
            f"centrality={float(quality['centrality_score']):.6f} | "
            f"optical={float(quality['optical_score']):.6f} | "
            f"total={float(quality['total_score']):.12f}"
        )
    if position_selection["status"] == "ambiguous":
        print("z_selection_ambiguous", flush=True)
        return 2
    if position_selection["status"] != "selected":
        raise RuntimeError("No valid best Z-position candidate was available")
    selected_position_index = int(position_selection["best_index"])
    best_position = best_global_positions[selected_position_index]
    measurement_qc_stack = load_stack(paths[measurement])
    measurement_window_qc = inspect_measurement_channel_windows(
        measurement_qc_stack,
        fixed_z_windows,
    )
    measurement_qc_by_z_mode = {
        result.z_mode: result for result in measurement_window_qc
    }
    for result in measurement_window_qc:
        reasons = ",".join(result.invalid_reasons) if result.invalid_reasons else "none"
        print_terminal_event(
            f"Window {result.position_1based}/5 | "
            f"Z={result.z0 + 1}-{result.z1 + 1} | "
            f"{measurement} technical QC | status={result.status} | "
            f"endpoints={result.lower_endpoint_fraction}/"
            f"{result.upper_endpoint_fraction} | "
            f"constant_slices={result.constant_slice_count}/"
            f"{result.observed_slices} | "
            f"sharpness={result.normalized_spatial_sharpness} | "
            f"background_nonuniformity="
            f"{result.normalized_background_nonuniformity} | "
            f"reasons={reasons}"
        )
    for global_position in best_global_positions:
        result = measurement_qc_by_z_mode[chosen_specs[global_position].z_mode]
        rows[global_position]["measurement_channel_qc_status"] = result.status
        rows[global_position]["measurement_channel_qc"] = asdict(result)
    for row in rows:
        row["auto_selected"] = False
    for global_position in best_global_positions:
        rows[global_position]["z_position_selected"] = False
    rows[best_position]["auto_selected"] = True
    rows[best_position]["z_position_selected"] = True
    selected_measurement_qc = measurement_qc_by_z_mode[chosen_specs[best_position].z_mode]
    if selected_measurement_qc.status != "valid":
        reasons = ", ".join(selected_measurement_qc.invalid_reasons)
        raise RuntimeError(
            f"Selected Z position has invalid {measurement} technical QC: {reasons}"
        )
    ...
    print(
        f"Auto-selected position {selected_position_index + 1}/5, "
        f"candidate {best_row['candidate']:02d}: "
        f"position_score={best_row['z_position_total_score']}, "
        f"candidate_score={best_row['auto_selection_score']}, "
    ...
    del seen_base_keys, completed_by_window, position_by_z_mode, window_by_z_mode
    del position_best_candidates, best_global_positions, local_selection_details
    del measurement_qc_stack, measurement_window_qc, measurement_qc_by_z_mode
    ...
    pixel_width_um = float(metadata["DAPI"]["pixel_width_um"])
    pixel_height_um = float(metadata["DAPI"]["pixel_height_um"])
    age_parameter_set = age_profile_parameter_set_for_channels(structural_channels)
    age_parameter_config = AGE_PROFILE_PARAMETER_SETS[age_parameter_set]
    if filename_age_decision is not None:
        age_profile_decision = replace(
            filename_age_decision,
            threshold=float(age_parameter_config["threshold"]),
            parameter_set=age_parameter_set,
            weights=tuple(
                (key, float(value))
                for key, value in age_parameter_config["weights"].items()
            ),
        )
    else:
        age_profile_decision = classify_age_profile(
            best_mask,
            dapi_projection,
            best_struct,
            pixel_width_um,
    ...
    age_weights_text = ", ".join(
        f"{key}={value:.2f}" for key, value in age_profile_decision.weights
    )
    if age_profile_decision.source == "morphology_classifier":
        print(
            ...
            f"parameter_set={age_profile_decision.parameter_set}, "
            f"weights={age_weights_text})",
            flush=True,
        )
    else:
        print(
            f"Age profile: {age_profile_decision.profile} "
            f"(filename, parameter_set={age_profile_decision.parameter_set}, "
            f"weights={age_weights_text})",
            flush=True,
        )
    z0 = int(best_row["z_start_0based"])
    z1 = int(best_row["z_end_0based_inclusive"])
    nucleus_3d_context = None
    if structural_stacks:
        ...
        if dapi_depth_um is not None and structural_depth_um is not None:
            ...
            nucleus_3d_context = Nucleus3DContext(
                dapi_stack=dapi_stack,
    ...
    whole_labels, soma_labels, process_labels, compartment_metrics = (
        split_astrocyte_compartments_for_profile(
            ...
            nucleus_3d_context=nucleus_3d_context,
    ...
    compartment_metrics["adaptation"] = (
        (
            ...
            if nucleus_3d_context is not None
        ...
        if age_profile_decision.profile == "mature"
            ...
            f"{nucleus_3d_context.structural_channel} ownership, mandatory multi-nucleus "
    ...
    if args.skip_fiji:
        ...
        np.savez_compressed(
            ...
            age_profile_parameter_set=np.asarray(age_profile_decision.parameter_set),
            age_profile_weight_names=np.asarray(
                [key for key, _value in age_profile_decision.weights]
            ),
            age_profile_weights=np.asarray(
                [value for _key, value in age_profile_decision.weights],
                dtype=float,
            ),
            age_profile_threshold=np.asarray(age_profile_decision.threshold),
