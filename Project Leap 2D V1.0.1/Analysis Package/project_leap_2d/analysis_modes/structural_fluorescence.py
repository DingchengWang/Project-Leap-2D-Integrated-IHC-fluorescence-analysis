...
def age_profile_parameter_set_for_channels(
    structural_channels: list[str] | tuple[str, ...],
) -> str:
    """Select calibrated age parameters without changing Whole or Z selection."""

    channel_set = set(structural_channels)
    if channel_set == {"eGFP"}:
        return "egfp_only_p3_mature_calibration_v1"
    if channel_set == {"eGFP", "GFAP"}:
        return "egfp_gfap_legacy_v1"
    raise ValueError(
        "Age classification requires eGFP-only or eGFP+GFAP structural channels; "
        f"found {sorted(channel_set)}"
    )
...
def classify_age_profile(
    ...
    parameter_set: str = "egfp_gfap_legacy_v1",
) -> AgeProfileDecision:
    """Return a deterministic binary morphology profile from structural channels only."""

    try:
        parameter_config = AGE_PROFILE_PARAMETER_SETS[parameter_set]
    except KeyError as exc:
        raise ValueError(f"Unknown age-profile parameter set: {parameter_set}") from exc
    bounds = parameter_config["evidence_bounds"]
    ...
    evidence = {
        "broad_structure": _linear_evidence(
            median_width_um, *bounds["broad_structure"]
        ),
        "solid_structure": _linear_evidence(
            median_solidity, *bounds["solid_structure"]
        ),
        "limited_thin_arbor": 1.0
        - _linear_evidence(median_thin_fraction, *bounds["limited_thin_arbor"]),
        "limited_branching": 1.0
        - _linear_evidence(median_branch_density, *bounds["limited_branching"]),
        "polarized_shape": _linear_evidence(
            median_axis_ratio, *bounds["polarized_shape"]
        ),
        "multi_nucleus_overlap": _linear_evidence(
            multi_nucleus_fraction, *bounds["multi_nucleus_overlap"]
        ),
    }
    neonatal_score = (
        weights["broad_structure"] * evidence["broad_structure"]
        + weights["solid_structure"] * evidence["solid_structure"]
        + weights["limited_thin_arbor"] * evidence["limited_thin_arbor"]
        + weights["limited_branching"] * evidence["limited_branching"]
        + weights["polarized_shape"] * evidence["polarized_shape"]
        + weights["multi_nucleus_overlap"] * evidence["multi_nucleus_overlap"]
    )
    threshold = float(parameter_config["threshold"])
    ...
    return AgeProfileDecision(
        ...
        weights=tuple((key, float(value)) for key, value in weights.items()),
    )
...
def split_astrocyte_compartments_for_profile(
    ...
    pixel_height_um: float,
    profile: str,
    nucleus_3d_context: Nucleus3DContext | None = None,
    ...
    if profile == "mature":
        ...
        labels, soma_labels, process_labels, metrics = split_astrocyte_compartments(
            ...
            ownership_pixel_depth_um=(
                nucleus_3d_context.pixel_depth_um
                if nucleus_3d_context is not None
                else None
            ),
    ...
    validation_method = (
        ...
        f"{nucleus_3d_context.structural_channel} nucleus ownership and anchor gate + "
        if nucleus_3d_context is not None
