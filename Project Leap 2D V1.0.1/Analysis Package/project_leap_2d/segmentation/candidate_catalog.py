...
def candidate_profile_name(spec: CandidateSpec) -> str:
    match = re.search(r"_z\d{2}_(.+)$", spec.name)
    if match is None:
        raise ValueError(f"Candidate name does not encode a profile: {spec.name}")
    return match.group(1)

def candidate_profile_family(spec: CandidateSpec) -> str:
...
def candidate_module_name(spec: CandidateSpec) -> str:
    profile_name = candidate_profile_name(spec)
...
def candidate_module_display_name(spec: CandidateSpec) -> str:
    return {
...
def candidate_threshold_display_name(spec: CandidateSpec) -> str:
