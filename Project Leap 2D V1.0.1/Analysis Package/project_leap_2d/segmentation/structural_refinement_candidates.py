...
def morphology_with_structural_refinement_specs(
    ...
    """Retain 30 baselines and append six refinements per fixed Z position."""
    validate_fixed_z_windows(windows)
    morphology_baseline_candidates = morphology_baseline_specs(windows)
    ...
    for window in windows:
        window_candidates: list[CandidateSpec] = []
        for template, profile in added_profiles:
            ...
            window_candidates.append(
                replace(
                    template,
                    name=(
                        f"structural_refinement_z{window.position_1based:02d}_{variant}"
                    ),
        ...
        fine_process = next(
            spec
            for spec in window_candidates
            if candidate_profile_name(spec) == "fine_process_guarded"
        )
        window_candidates.extend(
            [
                replace(
                    fine_process,
                    name=(
                        "structural_refinement_"
                        f"z{window.position_1based:02d}_channel_consensus_guarded"
                    ),
                ...
                replace(
                    ...
                    name=(
                        "structural_refinement_"
                        f"z{window.position_1based:02d}_topology_continuity_guarded"
                    ),
        ...
        if len(window_candidates) != 6:
            raise AssertionError(
                f"Z position {window.position_1based} must contain six structural refinements"
            )
        added.extend(window_candidates)
    ...
    for window in windows:
        ...
        if len(group) != PRE_DISTRIBUTION_BASELINE_PROFILES_PER_Z:
            raise AssertionError(
                "Z position "
                f"{window.position_1based} must contain exactly "
