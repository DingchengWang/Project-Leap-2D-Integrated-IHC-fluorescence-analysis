...
def dapi_supported_anchor(dapi_proj: np.ndarray, struct: np.ndarray, candidate: np.ndarray, spec: CandidateSpec) -> np.ndarray:
...
def qc_metrics(mask: np.ndarray, struct: np.ndarray, dapi_proj: np.ndarray, spec: CandidateSpec) -> dict:
    labels = measure.label(mask)
...
def build_candidate_window_contexts(
    *,
    chosen_specs: list[CandidateSpec],
    structural_channels: list[str],
    ...
    def report_progress(
        *,
        ...
        if progress_callback is None:
            return
        match = re.fullmatch(r"fixed_z(\d{2})_(\d+)_(\d+)", spec.z_mode)
        if match is None:
            raise ValueError(f"Unknown fixed Z-window mode: {spec.z_mode}")
        event = {
            "position_1based": int(match.group(1)),
            "z_mode": spec.z_mode,
            "z_start_1based": z0 + 1,
    ...
    reported_projection_keys: set[tuple[int, int, str]] = set()
    reported_structural_keys: set[tuple] = set()
    for spec in chosen_specs:
        z0, z1 = z_range_from_mode(spec.z_mode)
        ...
        if projection_key not in reported_projection_keys:
            report_progress(
                spec=spec,
                z0=z0,
                z1=z1,
            ...
            reported_projection_keys.add(projection_key)
        ...
        if structural_key not in reported_structural_keys:
            report_progress(
                spec=spec,
                z0=z0,
                z1=z1,
                step="structural_composite",
            )
            reported_structural_keys.add(structural_key)
...
def candidate_precompute_jobs(
    *,
    ...
    input_dir: Path,
    structural_channels: list[str],
) -> list[tuple[int, dict, object, tuple]]:
    jobs: list[tuple[int, dict, object, tuple]] = []
    for context, specs in context_groups:
        representative = specs[0]
        match = re.fullmatch(
            r"fixed_z(\d{2})_(\d+)_(\d+)",
            representative.z_mode,
        )
        if match is None:
            raise ValueError(
                f"Unknown fixed Z-window mode: {representative.z_mode}"
            )
        position_1based = int(match.group(1))
        base_metadata = {
            "position_1based": position_1based,
            "z_mode": representative.z_mode,
            "z_start_1based": context.projection_key[0] + 1,
            "z_end_1based_inclusive": context.projection_key[1] + 1,
        }

        def metadata(step: str, **details) -> dict:
            return {**base_metadata, "step": step, **details}
        ...
        if any(spec.method == "log1p_gmm" for spec in specs):
            jobs.append(
                (
                    0,
                    metadata("gmm"),
                    precompute_distribution_models,
                    (context,),
                )
            )
...
def candidate_base_cache_key(struct: np.ndarray, spec: CandidateSpec) -> tuple:
