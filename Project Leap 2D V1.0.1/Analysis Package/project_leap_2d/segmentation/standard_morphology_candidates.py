...
def morphology_baseline_template_specs() -> list[CandidateSpec]:
    variants = [
        (
            "process",
            dict(
    ...
    for variant, parameters in variants:
        candidates.append(
            make_morphology_baseline_spec(
                f"morphology_baseline_template_{variant}",
                "template",
...
def validate_fixed_z_windows(windows: list[ZWindowPlan]) -> None:
    if len(windows) != EXPECTED_Z_INTERVAL_COUNT:
        raise ValueError(
            f"Expected {EXPECTED_Z_INTERVAL_COUNT} fixed Z windows, got {len(windows)}"
        )
    positions = [window.position_1based for window in windows]
    if positions != list(range(1, EXPECTED_Z_INTERVAL_COUNT + 1)):
        raise ValueError(f"Fixed Z-window positions are invalid: {positions}")
    modes = [window.z_mode for window in windows]
    if len(set(modes)) != EXPECTED_Z_INTERVAL_COUNT:
        raise ValueError("Fixed Z-window modes must be unique")
    for window in windows:
        if window.z1 < window.z0:
            raise ValueError(f"Fixed Z-window bounds are invalid: {window}")
        if window.z1 - window.z0 + 1 != window.window_slices:
            raise ValueError(f"Fixed Z-window size is inconsistent: {window}")
        if z_range_from_mode(window.z_mode) != (window.z0, window.z1):
            raise ValueError(f"Fixed Z-window mode is inconsistent: {window}")

def morphology_baseline_specs(windows: list[ZWindowPlan]) -> list[CandidateSpec]:
    """Build six unchanged morphology profiles for each fixed Z position."""
        ...
        for template, profile in profile_templates:
            ...
            candidates.append(
                replace(
                    template,
                    name=(
                        f"morphology_baseline_z{window.position_1based:02d}_{variant}"
                    ),
